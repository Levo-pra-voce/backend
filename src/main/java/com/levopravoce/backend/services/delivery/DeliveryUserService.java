...
public class DeliveryUserService implements UserManagement {
    ...
    public JwtResponseDTO save(UserDTO userDTO) {
        ...
        User user = User.builder().cpf(userDTO.getCpf()).cnh(userDTO.getCnh()).name(userDTO.getName()).email(userDTO.getEmail()).password(passwordEncoder.encode(userDTO.getPassword())).contact(userDTO.getPhone()).status(Status.ACTIVE).creationDate(LocalDateTime.now()).userType(UserType.ENTREGADOR).addresses(List.of(userUtils.buildAddressByUserDTO(userDTO))).vehicles(List.of(Objects.requireNonNull(vehicle))).build();
    ...
    public void update(User currentUser, UserDTO updatedUserDTO) {
        ...
        if (!Objects.equals(updatedUserDTO.getVehicle(), currentUser.getVehicles().stream().filter(Vehicle::isActive).findFirst().orElse(null))) {
